...
class LivestockDetails {
    ...
    static async create(livestockData) {
        const {
            user_id, cow, buffalo, goat, sheep, pig, poultry, horse, others
        } = livestockData;
    ...
    static async update(user_id, livestockData) {
        const { cow, buffalo, goat, sheep, pig, poultry, horse, others } = livestockData;
        ...
        const values = [user_id, cow, buffalo, goat, sheep, pig, poultry, horse, others];
        const result = await query(text, values);
        return result.rows[0];
    }
    ...
    static async getFarmersWithLocations() {
        ...
        const gpsText = `
            SELECT
                ...
                ld.cow, ld.buffalo, ld.goat, ld.sheep, ld.pig, ld.poultry, ld.horse, ld.others
    ...
    static async getStatistics() {
        const text = `
            SELECT 
                ...
                SUM(horse) as total_horses,
                SUM(others) as total_others,
                AVG(cow + buffalo + goat + sheep + pig + poultry + horse + others) as avg_livestock_per_farmer
    ...
    static async findByLivestockType(livestockType, minCount = 1) {
        const validTypes = ['cow', 'buffalo', 'goat', 'sheep', 'pig', 'poultry', 'horse', 'others'];
    ...
    static async createWithClient(client, livestockData) {
        const { user_id, cow, buffalo, goat, sheep, pig, poultry, horse, others } = livestockData;
        const text = `
            INSERT INTO public.livestock_details (
                user_id, cow, buffalo, goat, sheep, pig, poultry, horse, others
            ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
            RETURNING *
        `;
        const values = [user_id, cow || 0, buffalo || 0, goat || 0, sheep || 0, pig || 0, poultry || 0, horse || 0, others || 0];
    ...
    static async updateWithClient(client, user_id, livestockData) {
        ...
        const text = `
            ...
            SET 
                ...
                pig = COALESCE($6, pig),
                poultry = COALESCE($7, poultry),
                horse = COALESCE($8, horse),
                others = COALESCE($9, others),
                updated_at = timezone('utc', now())
            WHERE user_id = $1
            RETURNING *
        `;
